...
//   VIKSOFT_API_BASE      (обов'язково, HTTPS у production)
...
// Скільки EIC опитувати ОДНОЧАСНО в межах одного запиту закладу.
// УВАГА: за замовчуванням = 1 (ПОСЛІДОВНО). Розробник Vik-Soft повідомив, що при
// кількох одночасних запитах на отримання лічильників його модуль зависає, тому
// надсилаємо запити по одному. За потреби можна підняти через env
// VIKSOFT_EIC_CONCURRENCY (напр. 2-3), якщо модуль витримує паралелізм.
const EIC_FETCH_CONCURRENCY = Math.max(
  1,
  Number.parseInt(String(process.env.VIKSOFT_EIC_CONCURRENCY || "1"), 10) || 1
);
